...
static void malloc_init() { /* grab the last valid address from the OS*/
  last_valid_address = (void*)(__builtin_wasm_memory_size(0) * (64 << 10));
  /* we don't have any memory to manage yet, so just set the beginning to be
  last_valid_address */
...
void *malloc(unsigned long numbytes) {
  ...
  if (!memory_location) {
    unsigned long pages = numbytes / (64 << 10);
    if (numbytes % (64 << 10) != 0)
    {
      pages++;
      numbytes = pages * (64 << 10);
    }
